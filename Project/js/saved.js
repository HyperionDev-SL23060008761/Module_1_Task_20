...
commentListElement.addEventListener("click", handleCommentClickEvents);
...
function handleCardClickEvents(clickEvent) {
	...
	//Check if the Target Element is Invalid
	...
	//const setup the variable to check if a card's Watched Status should be toggled
	const shouldToggleWatchedStatus = !!targetElement.closest(".watched_bar");
	...
	//Check if the Card's Watched Status should be Toggle
	if (shouldToggleWatchedStatus) return toggleWatchedStatus(targetElement);
}

/**
 * Handles Click Events for a Comment
 * @param {PointerEvent} clickEvent
 */
function handleCommentClickEvents(clickEvent) {
	//
	//Get the Target Element
	const targetElement = clickEvent.target;
	...
	//Get the Target Comment Element
	const targetCommentElement = targetElement.closest(".comment");

	//Check if the Target Comment Element is Invalid
	if (!targetCommentElement) return;

	//Get the Target Comment's Content Element
	const targetCommentContentElement = targetCommentElement.querySelector(".comment_content");

	//Toggle Hiding the Comment's Content
	$(targetCommentContentElement).slideToggle();
}
...
function deleteCard(targetElement) {
	...
	//Notify the User of the Successful Deletion of the Anime
	return notifier.notify(
		"Success",
		`Deleted Anime from your list (${cardList.length} in List)`,
		2500
	);
}
...
function addCard(newCardModal, clickEvent) {
	...
	//Check if the Card List is Invalid
	if (!cardList) return notifier.notify("Fail", "Failed to Add Card", 2500);
	...
	//Notify the user of the addition of the card
	return notifier.notify(
		"Success",
		`Added new Anime to your list (${cardList.length + 1} in List)`,
		2500
	);
}
...
/**
 * Toggles the Watch Status for a Selected Card
 * @param {EventTarget} targetElement
 */
function toggleWatchedStatus(targetElement) {
	//
	//Get the Parent Card Element
	const cardElement = targetElement.closest(".card");
	...
	//Get the Card Element's Watched Element
	const cardWatchedIconElement = cardElement.querySelector(".watched_icon");

	//Get the Card Element's No Watched Icon
	const cardNoWatchedIconElement = cardElement.querySelector(".no_watched_icon");

	//Update the Card's Watched Status
	card.watched = !card.watched;

	//Update the Card's Watched Icon Element
	cardWatchedIconElement.classList.toggle("watched_icon_hidden", !card.watched);

	//Update the Card's No Watched Icon Element
	cardNoWatchedIconElement.classList.toggle("watched_icon_hidden", card.watched);
